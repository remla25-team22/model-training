...
def test_input_data_columns():
    df = pd.read_csv('data/a1_RestaurantReviews_HistoricDump.tsv', delimiter='\t', quoting=3)
...
def test_model_output_shape():
    ...
    model = joblib.load('models/c2_Classifier_Sentiment_Model')
    X = cv.transform([clean_review(r) for r in sample_reviews]).toarray()
    y_pred = model.predict(X)
    assert y_pred.shape[0] == 2

def test_model_files_exist():
    assert os.path.exists('models/c1_BoW_Sentiment_Model.pkl')
    assert os.path.exists('models/c2_Classifier_Sentiment_Model')
